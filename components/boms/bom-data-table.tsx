...
interface DataTableProps<TData extends BomVersionGroup, TValue> {
  columns: ColumnDef<TData, TValue>[];
  data: TData[];
}

export function BomDataTable<TData extends BomVersionGroup, TValue>({
  columns,
  data,
  ...
  const multiWordFilterFn: FilterFn<TData> = (row, _columnId, filterValue) => {
    ...
    const bomCode = row.original.bomCode.toLowerCase();
    
    const allMaterialsText: string[] = [];
    row.original.versions.forEach((materials) => {
      const versionMaterialsText = materials
        .map(
          (m) =>
            `${m.material.toLowerCase()} ${m.materialDescription.toLowerCase()} ${m.partReference.toLowerCase()}`
        )
        .join(" ");
      allMaterialsText.push(versionMaterialsText);
    });
    
    const rowText = `${bomCode} ${allMaterialsText.join(" ")}`;
            ...
            <Input
              ...
              placeholder={
                filterChips.length === 0
                  ? "Cari (BOM Code, Material, Deskripsi...), lalu tekan Enter..."
                  : "Tambah filter..."
              }
              value={inputValue}
...
<div className="flex flex-col items-center justify-between gap-4 py-4 sm:flex-row">
        ...
        <div className="flex mb-12 flex-col-reverse items-center gap-4 sm:flex-row sm:gap-6">
