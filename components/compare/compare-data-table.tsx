...
import { ComparisonView } from "@/lib/types";
...
interface DataTableProps<TData extends ComparisonView, TValue> {
  columns: ColumnDef<TData, TValue>[];
  data: TData[];
}

export function CompareDataTable<TData extends ComparisonView, TValue>({
  columns,
  data,
  ...
  const multiWordFilterFn: FilterFn<TData> = (row, _columnId, filterValue) => {
    ...
    const rowText = [
      row.original.bomCode,
      row.original.wbsNumber?.String,
      row.original.compartmentNumber,
      row.original.switchboardName,
    ].join(" ").toLowerCase();
            ...
            <Input
              ...
              placeholder={
                filterChips.length === 0
                  ? "Cari (BOM Code, WBS, Kompartemen...), lalu tekan Enter..."
                  : "Tambah filter..."
              }
              value={inputValue}
