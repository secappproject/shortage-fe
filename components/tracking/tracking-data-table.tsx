...
import { ProjectTracking } from "@/lib/types";
...
interface DataTableProps<TData extends ProjectTracking, TValue> {
  columns: ColumnDef<TData, TValue>[];
  data: TData[];
}

export function TrackingDataTable<TData extends ProjectTracking, TValue>({
  columns,
  data,
  ...
  const multiWordFilterFn: FilterFn<TData> = (row, _columnId, filterValue) => {
    ...
    const rowText = row
      ...
      .map((cell) => {
        ...
        if (typeof cellValue === "object" && cellValue && "Valid" in cellValue) {
          return (cellValue as { String: string; Valid: boolean }).String || "";
        }
            ...
            <Input
              ...
              placeholder={
                filterChips.length === 0
                  ? "Cari (WBS, Project, Switchboard, Part...), lalu tekan Enter..."
                  : "Tambah filter..."
              }
              value={inputValue}
